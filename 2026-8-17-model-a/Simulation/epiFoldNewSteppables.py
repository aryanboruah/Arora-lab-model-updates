   ...
   def step(self, mcs):
        ...
        if mcs>=0 and mcs%50 == 0:    
            ...
            for cell in self.cell_list_by_type(self.STROMA):
                ...
                # if mcs>=0 and mcs%50 == 0:
                    if 0 < cell.xCOM < 800 and 230 < cell.yCOM < 278:
                    # if 380 < cell.xCOM < 420:
                        direction = -1 if cell.xCOM >= fold_center else 1              
                        cell.lambdaVecX = direction * force
...
class forceGradient(SteppableBasePy):
    ...
    def start(self):
        ...
        self.stromaCell_force     = params['forces']
        self.stromaDecay          = params['decays']
    ...
    def step(self, mcs):
        ...
        if mcs>=50 and mcs%50==0:                         # STROMA MOVING INWARD TO FORM THE FOLD 
                ...
                # for stroma
                F0 = self.stromaCell_force[6]                              # BASE (PEAK) LAMBDAVECY AT A FOLD CENTER
                decay_rate = self.stromaDecay[4]                          # EXPONENTIAL DECAY RATE (LARGER -> FASTER DECAY)
